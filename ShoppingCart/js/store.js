...
function store() {
    ...
    this.vegetables = [
        new product("APL", "LadyFinger", "Eat one every day to keep the doctor away!", 12, 90, 0, 2, 0, 1, 2),
        new product("AVC", "Carrot", "Guacamole anyone?", 16, 90, 0, 1, 1, 1, 2),
        new product("BAN", "Tomato", "These are rich in Potassium and easy to peel.", 4, 120, 0, 2, 1, 2, 2),
        new product("CTP", "Potato", "Delicious and refreshing.", 3, 50, 4, 4, 1, 2, 0),

    ];
    this.grocery = [
        new product("APL", "Table", "Eat one every day to keep the doctor away!", 12, 90, 0, 2, 0, 1, 2),
        new product("AVC", "Chair", "Guacamole anyone?", 16, 90, 0, 1, 1, 1, 2),
        new product("BAN", "Yogurt", "Eat everyday", 4, 120, 0, 2, 1, 2, 2),
        new product("CTP", "Surf", "Clean Clothes", 3, 50, 4, 4, 1, 2, 0),
...
store.prototype.getProduct = function (sku) {
    for (var i = 0; i < this.fruits.length; i++) {
        if (this.fruits[i].sku == sku)
            return this.fruits[i];
    }
    return null;
}

store.prototype.getProductList = function (name) {
    if(name!= null)
    {
        if(name === 'Fruits')
        {
